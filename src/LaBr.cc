...
namespace FDSi {
  LaBrConf::LaBrConf(std::string na, std::string conffile) {
    conf_name = na; 
  ...
  int LaBrConf::ReadCal(std::string calfile) {
    ...
    while (std::getline(file, line)) {
      ...
      int id, i;
      ss >> id >> i;
      ss >> offset[id][i] >> gain[id][i] >> quad[id][i] >> toff[id][i];
      ++n;
    }
    return n;
  }

  int LaBrConf::ReadCTCal(std::string calfile) {
    std::ifstream file(calfile.c_str());
    if (!file.is_open()) {
      std::cout << "Warning! " << calfile << " not open" << std::endl;
      return -1;
  ...
  int LaBrConf::ReadAngleMap(std::string mapfile) {
  ...
  int LaBrConf::ReadEfficiencyCal(std::string fn) {
  ...
  int LaBrConf::ReadEffIDCal(std::string fn) {
    std::ifstream file(fn.c_str());
    if (!file.is_open()) {
      std::cout << "Warning! " << fn << " not open" << std::endl;
      return -1;
  ...
  float LaBrConf::Efficiency(const double &e) const {
  ...
  float LaBrConf::Efficiency(const double &e, const int &ID) const {
    double x = std::log(e/100.0);
    double y = std::log(e/1000.0);
  ...
  void LaBrConf::Print() {
    ...
    for (int i=0; i<nClovers; ++i) {
      if (present[CloverID[i]] == 0) { continue; }
      for (int j=0; j<nSubDets[i]; ++j) {
        printf("%i  %i     %5.4f    %5.4f   %9.8f    %5.4f\n", CloverID[i], j, offset[CloverID[i]][j], gain[CloverID[i]][j], quad[CloverID[i]][j], toff[CloverID[i]][j]);
      }
    }
    if (ct_cal_loaded) {
  ...
  void LaBrArray::AddHit(const PIXIE::Measurement &meas, int id, int cryst) {
    if (nHits >= MAX_HITS) {
